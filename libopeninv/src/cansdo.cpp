...
#define PRINT_BUF_ENQUEUE(c)  do { uint32_t _i = printByteIn; printByteIn = _i + 1; printBuffer[_i & (sizeof(printBuffer) - 1)] = c; } while(0)
#if __cplusplus >= 201103L
#define PRINT_BUF_DEQUEUE()   [&]{ uint32_t _i = printByteOut; printByteOut = _i + 1; return printBuffer[_i & (sizeof(printBuffer) - 1)]; }()
#else
   ...
   else if (canId == (SDO_REP_ID_BASE + remoteNodeId))
   {
      ...
      if (sdoFrame->index == SDO_INDEX_MAP_RX || sdoFrame->index == SDO_INDEX_MAP_TX)
      {
         if (sdoFrame->subIndex == 0)
            InitiateSDOTransfer(SDO_WRITE, remoteNodeId, sdoFrame->index, 1, mapInfo.mapParam | ((uint32_t)mapInfo.offsetBits << 16) | ((uint32_t)mapInfo.numBits << 24));
         else if (sdoFrame->subIndex == 1)
            InitiateSDOTransfer(SDO_WRITE, remoteNodeId, sdoFrame->index, 2, (int32_t)(mapInfo.gain * 1000.0f) | ((uint32_t)mapInfo.offset << 24));
      }
...
{
   // Copy the packed struct into an aligned buffer before passing to Send(),
   // which takes uint32_t*. Casting a packed struct pointer directly is UB
   // and triggers -Waddress-of-packed-member on GCC.
   uint32_t buf[2];
   const uint8_t *src = (const uint8_t*)sdoFrame;
   uint8_t *dst = (uint8_t*)buf;
   for (int i = 0; i < (int)sizeof(SdoFrame); i++) dst[i] = src[i];
