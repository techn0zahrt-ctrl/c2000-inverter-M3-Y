   ...
   if (0 != recvMap)
   {
      forEachPosMap(curPos, recvMap)
      {
         ...
         if (curPos->numBits < 0) //negative length is big endian
         {
            ...
            word = ((uint32_t)bptr[0] << 24) | ((uint32_t)bptr[1] << 16) | ((uint32_t)bptr[2] << 8) | bptr[3];
            pos = 31 - pos;
         }
         else //little endian
         {
   ...
   forEachCanMap(curMap, canSendMap)
   {
      ...
      forEachPosMap(curPos, curMap)
      {
         ...
         if (curPos->numBits < 0) // big-endian
         {
            ...
            ival = ((uint32_t)bptr[0] << 24) | ((uint32_t)bptr[1] << 16) | ((uint32_t)bptr[2] << 8) | bptr[3];
...
void CanMap::Save()
{
   ...
   uint32_t *checkAddress = (uint32_t*)(uintptr_t)baseAddress;

   isSaving = true;

   for (uint32_t i = 0; i < FLASH_PAGE_SIZE / 4; i++, checkAddress++)
      check &= *checkAddress;
...
{
   uint32_t baseAddress = GetFlashAddress();
   uint32_t storedCrc = *(uint32_t*)(uintptr_t)CRC_ADDRESS(baseAddress);
   uint32_t crc;

   crc_reset();
   crc = crc_calculate_block((uint32_t*)(uintptr_t)baseAddress, SENDMAP_WORDS + RECVMAP_WORDS + POSMAP_WORDS);

   if (storedCrc == crc)
   {
      memcpy32((int*)canSendMap, (int*)(uintptr_t)SENDMAP_ADDRESS(baseAddress), SENDMAP_WORDS);
      memcpy32((int*)canRecvMap, (int*)(uintptr_t)RECVMAP_ADDRESS(baseAddress), RECVMAP_WORDS);
      memcpy32((int*)canPosMap, (int*)(uintptr_t)POSMAP_ADDRESS(baseAddress), POSMAP_WORDS);
      ReplaceParamUidByEnum(canSendMap);
      ReplaceParamUidByEnum(canRecvMap);
      return 1;
   }
...
/** \brief Convert a legacy CAN map array into the current map format.
 *
 * Extracted from LegacyLoadFromFlash to avoid a C++11 lambda — the TI
 * C++03 compiler does not support lambda expressions. Declared as a private
 * static member so it can access the private Add() method via self.
 */
void CanMap::ConvertLegacyMap(CanMap* self, LEGACY_CANIDMAP* m, CANIDMAP* newIdMap,
                               bool gainToFloat, int legacyMaxMessages,
                               int maxItemsPerMessage, int canidUnset)
{
   for (LEGACY_CANIDMAP *c = m; (c - m) < legacyMaxMessages && c->canId < (uint16_t)canidUnset; c++)
   {
      for (LEGACY_CANPOS *cp = c->items; (cp - c->items) < maxItemsPerMessage && cp->numBits > 0; cp++)
      {
         Param::PARAM_NUM param = Param::NumFromId(cp->mapParam);
         self->Add(newIdMap, param, c->canId, cp->offsetBits, cp->numBits,
                   gainToFloat ? FP_TOFLOAT(cp->gain) : cp->gain, 0);
      }
   }
}
...
int CanMap::LegacyLoadFromFlash()
{
   ...
   uint32_t storedCrc = *(uint32_t*)(uintptr_t)(data + size);

   crc_reset();
   uint32_t crc = crc_calculate_block((uint32_t*)(uintptr_t)data, size / 4);

   if (storedCrc == crc)
   {
      ConvertLegacyMap(this, (LEGACY_CANIDMAP*)(uintptr_t)data, canSendMap, false,
                       LEGACY_MAX_MESSAGES, MAX_ITEMS_PER_MESSAGE, CANID_UNSET);
      ConvertLegacyMap(this, (LEGACY_CANIDMAP*)(uintptr_t)(data + sizeof(LEGACY_CANIDMAP) * LEGACY_MAX_MESSAGES),
                       canRecvMap, true,
                       LEGACY_MAX_MESSAGES, MAX_ITEMS_PER_MESSAGE, CANID_UNSET);
      return 1;
   }
